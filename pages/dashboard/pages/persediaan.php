...
if(isset($_POST['tambah_barang'])) {
	...
	$cek = mysqli_query($conn, "SELECT * FROM persediaan WHERE nama_barang='$nama_barang'");

	if(!$cek->num_rows > 0) {
		$addtotable = mysqli_query($conn, "INSERT INTO persediaan (id_barang, nama_barang, harga_barang, stock_barang) VALUES (NULL, '$nama_barang', '$harga_barang', '$stock_barang')");
		if($addtotable) {
			echo "<script>alert('Yeay, Tambah Barang berhasil!')</script>";
			// header('location:persediaan.php');
			$nama_barang = "";
			$harga_barang = "";
			$stock_barang = "";			
		} else {
			echo "<script>alert('Yahh :( Tambah Barang gagal!')</script>";
			// header('location:persediaan.php');
		}
	} else {
...
if(isset($_POST['update_barang'])) {
	...
	$update_barang = mysqli_query($conn, "UPDATE persediaan SET nama_barang='$nama_barang', harga_barang='$harga_barang' where id_barang='$id_barang'");
	if($update_barang) {
		echo "<script>alert('Yeay, Edit Persediaan berhasil!')</script>";
	} else {
		echo "<script>alert('Yahh :( Edit Persediaan gagal!')</script>";
	}
}
...
if(isset($_POST['hapus_barang'])) {
	...
	$hapus_barang = mysqli_query($conn, "DELETE FROM persediaan WHERE id_barang='$id_barang'");
	if($hapus_barang) {
		echo "<script>alert('Yeay, Hapus Persediaan berhasil!')</script>";
	} else {
		echo "<script>alert('Yahh :( Hapus Persediaan gagal!')</script>";
	}
}
...
<html lang="en">
	...
	<head>
		...
		<title>Persediaan - Nio Furniture</title>
						...
						<ul>
							...
							<li class="submenu"> <a href="#"><i class="fas fa-cube"></i> <span> Persediaan </span> <span class="menu-arrow"></span></a>
								<ul class="submenu_class" style="display: none;">
									<li class="aktif"><a href="persediaan.php"> Persediaan </a></li>
									...
									<div class="table-responsive">
										<table class="datatable table table-stripped">
											...
											<tbody>
												...
												<?php
													$ambil_data_stock = mysqli_query($conn, "SELECT * FROM persediaan");
												...
												<tr>
													...
													<td><?=$nama_barang;?></td>
													<td><img src="" alt=""></td>
													<td><?=rupiah($harga_barang);?></td>
													<td><?=$stock_barang;?></td>
													<td class="text-right">
														<div class="dropdown dropdown-action"> <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="fas fa-ellipsis-v ellipse_color"></i></a>
															<div class="dropdown-menu dropdown-menu-right">
																<a class="dropdown-item" href="#editModal<?=$id_barang;?>" data-toggle="modal" data-target="#editModal<?=$id_barang;?>"><i class="fas fa-pencil-alt m-r-5"></i> Edit</a>
																<input type="hidden" name="id_barang_to_hapus" value="<?=$id_barang;?>">
																<a class="dropdown-item" href="#hapusModal<?=$id_barang;?>" data-toggle="modal" data-target="#hapusModal<?=$id_barang;?>"><i class="fas fa-trash-alt m-r-5"></i> Hapus</a>
															</div>
														</div>
													</td>
												</tr>
													...
													<!-- Modal Edit -->
													<div class="modal fade editModal" tabindex="-1" role="dialog" aria-labelledby="bannerformmodal" aria-hidden="true" id="editModal<?=$id_barang;?>">
														<div class="modal-dialog">
															<div class="modal-content">
															
																	<!-- Modal Header -->
																	<div class="modal-header">
																		<h4 class="modal-title">Edit Persediaan</h4>
																	...
																	<form method="POST">
																		<div class="modal-body">
																			...
																			<br>
																			<input type="number" name="harga_barang" value="<?=$harga_barang;?>" class="form-control" required>
																			<input type="hidden" name="id_barang" value="<?=$id_barang;?>">
																		</div>
																					
																		<!-- Modal footer -->
																		<div class="modal-footer">
																			<button type="submit" class="btn btn-primary" name="update_barang">Simpan</button>
																		</div>
																	</form>
																</div>
															</div>
													...
													<!-- Modal Hapus -->
													<div class="modal fade hapusModal" tabindex="-1" role="dialog" aria-labelledby="bannerformmodal" aria-hidden="true" id="hapusModal<?=$id_barang;?>">
														<div class="modal-dialog">
															<div class="modal-content">
																	...
																	<form method="POST">
																		<div class="modal-body">
																			<p>Apakah Anda yakin akan menghapus <b><?=$nama_barang;?></b> ?</p>
																			<input type="hidden" name="id_barang" value="<?=$id_barang;?>">
																		</div>
																					
																		<!-- Modal footer -->
																		<div class="modal-footer">
																			<button type="submit" class="btn btn-primary" name="hapus_barang">Hapus</button>
																		</div>
																	</form>
																</div>
															</div>
			...
			<div class="modal-content">
					...
					<form method="POST">
						<div class="modal-body">
							...
							<br>
							<input type="number" name="harga_barang" placeholder="Harga Barang" class="form-control" required>
							<br>
							<input type="number" name="stock_barang" placeholder="Stock Barang" class="form-control" required>
							<br>
						...
						<!-- Modal footer -->
						<div class="modal-footer">
							<button type="submit" class="btn btn-primary" name="tambah_barang">Simpan</button>
						</div>
					</form>
				</div>
			</div>
	...
	<!-- Modal Edit -->
	<div class="modal fade editModal" tabindex="-1" role="dialog" aria-labelledby="bannerformmodal" aria-hidden="true" id="editModal">
		<div class="modal-dialog">
			<div class="modal-content">
